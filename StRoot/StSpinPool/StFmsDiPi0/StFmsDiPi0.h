...
public: 
    ...
    void VpdTimingCutUp(int vpdCutUp) { mApplyVpdTimeUp = true; mVpdCutUp = vpdCutUp; }//added by Xiaoxuan
    void VpdTimingCutLow(int vpdCutLow) { mApplyVpdTimeLow = true; mVpdCutLow = vpdCutLow; }//added by Xiaoxuan
...
private:
    ...
    enum {kNPtBin=6,kNCut=26};//added by Xiaoxuan
    //enum {kNPtBin=2,kNCut=26};//Xiaoxuan
    ...
    TH1D* mTDC=0;
    TH1D* mTriggerCount;//added by Xiaoxuan, trigger distribution in event level

    TH1F* mPt0[kNCut+1];//added by Xiaoxuan
    ...
    //TH1F* mX1[kNPtBin][kNPtBin][kNCut+1];
    //TH1F* mX2[kNPtBin][kNPtBin][kNCut+1];
    TH1F* mBbce[kNPtBin][kNPtBin][kNCut+1];
    TH1F* mTofm[kNPtBin][kNPtBin][kNCut+1];
    TH2F* mPhi1Dphi[kNPtBin][kNPtBin][kNCut+1];
    //TH2F* mBXingDphi[kNPtBin][kNPtBin][kNCut+1];//Xiaoxuan

    char* mTreeFilename=0;
    int mWriteTree=0;
    int mReadTree=0;
    bool mApplyVpdTimeUp = false; //Turns to true if VpdTimingCut is used, added by Xiaoxuan
    bool mApplyVpdTimeLow = false;//Turns to true if VpdTimingCut is used, added by Xiaoxuan
    int mVpdCutUp = 0;//added by Xiaoxuan
    int mVpdCutLow = 0;//added by Xiaoxuan
    TFile* mTreeFile=0;
    TTree* mTree=0;
    TTree* mChain=0;

    virtual const char *GetCVS() const
    {static const char cvs[]="Tag $Name: SL18h $ $Id: StFmsDiPi0.h,v 1.5 2017/09/05 17:42:19 akio Exp $ built " __DATE__ " " __TIME__ ; return cvs;}
