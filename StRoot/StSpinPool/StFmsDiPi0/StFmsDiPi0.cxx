...
//const float mPtBin[7]={0.5,1.0, 1.4, 2.0, 2.4, 2.8, 5.0};//Xiaoxuan
const float mPtBin[7]={0.5,1.0,1.5,2.0,2.5,3.0,5.0};
...
TH1F *getLogXBin(TString histname, TString histaxis, 
		...
		TH1F *htemp = new TH1F(histname, histaxis, bin_num, x_edge);
		return htemp;
}
...
StFmsDiPi0::StFmsDiPi0(const Char_t* name):
	...
	Int_t StFmsDiPi0::Init(){  
		...
		if(mWriteTree){
			...
			//Add by Xiaoxuan, positions of trigger pion0 and associate pio
			mTDC = new TH1D("TDC","VpdWestTDC", 60, 0, 3000);
			mX1=getLogXBin("x1","x1;x1;counts",128,1e-4,1);
			mX2=getLogXBin("x2","x2;x2;counts",128,1e-4,1);
			...
			for(int i=0; i<kNPtBin; i++){
				for(int j=0; j<=i; j++){
					for(int k=0; k<=kNCut; k++){
						...
						//sprintf(c,"x1_%1d%1d_c%d",i,j,k); mX1[i][j][k]=getLogXBin(c,c,128,1e-4,1);
						//sprintf(c,"x2_%1d%1d_c%d",i,j,k); mX2[i][j][k]=getLogXBin(c,c,128,1e-4,1);
						...
						//sprintf(c,"bxingdphi_%1d%1d_c%d",i,j,k); mBXingDphi[i][j][k]=new TH2F(c,c,120,0.0,120.0,50,-0.5*PI,1.5*PI);//Xiaoxuan
					}
				}
			}
		}
...
Int_t StFmsDiPi0::Make(){
	...
	int bsSTrg1=0, bsSTrg2=0, bsSTrg3=0, bsLTrg1=0, bsLTrg2=0, bsLTrg3=0;//Xiaoxuan
	int bunch=-1, trigger=-1;
	int ptThdCut = 0;//Xiaoxuan, pt cut for trigger pi0, pt>threshold of each trigger
	...
	if(mPythia==1){
		...
		if(mReadTree==1){
			...
			/********Added by Xiaoxuan, apply west Vpd TDC cut in event level********/
			if (mApplyVpdTimeUp)
                	{
				const StTriggerData* triggerData = mudst->event()->triggerData();
                        	...
                        	if (vpdWestTdc > mVpdCutUp) return kStSkip;
                	}
			if (mApplyVpdTimeLow)
                	{
				const StTriggerData* triggerData = mudst->event()->triggerData();
                        	if (!triggerData) { LOG_ERROR <<"StFmsCalibMaker::Make - !triggerData" <<endl; return kStErr; }
                        	UShort_t vpdWestTdc = triggerData->vpdEarliestTDC(west);
                        	if (vpdWestTdc <= mVpdCutLow) return kStSkip;
                	}
			const StTriggerData* triggerData = mudst->event()->triggerData();
                       	UShort_t vpdWestTdc = triggerData->vpdEarliestTDC(west);
			mTDC->Fill(vpdWestTdc);

			/********Added by Xiaoxuan ends****************************************/
		...
		if     (bunch>=31  && bunch< 40) {ag=1;}
		else if(bunch>=111 && bunch<120) {ag=2;}   
		...
		//Xiaoxuan, trigger dependence
		if(trigger & 0x001) bsSTrg1=1;
                if(trigger & 0x002) bsSTrg2=1;
                if(trigger & 0x004) bsSTrg3=1;
                if(trigger & 0x008) bsLTrg1=1;
                if(trigger & 0x010) bsLTrg2=1;
                if(trigger & 0x020) bsLTrg3=1;
		//Xiaoxuan
	...
	if(pcut.size()>1){
		...
		if(pair.size()>0) {	
			...
			//cout<<"before write Tree file "<<endl;
			...
			if(mWriteTree){
				...
				cout<< "mWriteTree "<<mWriteTree<<endl;
				//cout<<"before fill tree "<<endl;
				mTree->Fill();
				//return kStOK; //exit if writing TTree
			}

			//cout<<" before sort pair"<<endl;
			...
			//cout<<" before loop 1st pair "<<endl;
			
			/****
			//Add by Xiaoxuan
			//check di pion0s positions with and without TDC cuts
			if(mWriteTree ){
				UShort_t tdc1west;
				StMuDst* mudst = (StMuDst*)GetInputDS("MuDst");
				if(!mudst) {LOG_ERROR << "StFmsDiPi0::Make did not find MuDst**************************"<<endm; return kStErr;}
				const StTriggerData* triggerData = mudst->event()->triggerData();
				if(!triggerData) {LOG_ERROR << "StFmsDiPi0::Make did not find Trigger**************************"<<endm; return kStErr;}
				tdc1west = triggerData->vpdEarliestTDC(west);
				cout<<"tdc1west "<<tdc1west<<endl;
				mTDC->Fill(tdc1west);
				//if(tdc1west <= 1300)
				//{
					if(pair.size() > 1 && pair[0]->pT() > 3 && pair[1]->pT() > 3
						&& pair[0]->mass() >= MassCut0 && pair[0]->mass() < MassCut1
						&& pair[1]->mass() >= MassCut0 && pair[1]->mass() < MassCut1)
					{
						mPion0 -> Fill(pair[0]->x(), pair[0]->y());
						mPion1 -> Fill(pair[1]->x(), pair[1]->y());
					}
				//}
			}
			//Add by Xiaoxuan ends
			****/
			for(unsigned int i=0; i<pair.size(); i++){ //loop over 1st pair
				...
				//if(ag<=3 && bsTrg==1     && exclusive1) cut1[17]=1;//Xiaoxuan, include all ag
				...
				//Xiaoxuan, trigger dependence
				if(ag==0 && bsSTrg1==1    && exclusive1) cut1[20]=1;
				if(ag==0 && bsSTrg2==1    && exclusive1) cut1[21]=1;
				if(ag==0 && bsSTrg3==1    && exclusive1) cut1[22]=1;
				if(ag==0 && bsLTrg1==1    && exclusive1) cut1[23]=1;
				if(ag==0 && bsLTrg2==1    && exclusive1) cut1[24]=1;
				if(ag==0 && bsLTrg3==1    && exclusive1) cut1[25]=1;
				//Xiaoxuan
				...
				//added by Xiaoxuan, require pt1>threshold for each trigger
				if( 
					((bsSTrg1==1 || bsLTrg1==1)&&pair[i]->pT()>1.4) || //bsSm1 and bsLg1
					((bsSTrg2==1 || bsLTrg2==1)&&pair[i]->pT()>2.0) ||//bsSm2 and bsLg2
					( bsSTrg3==1 && pair[i]->pT()>2.4) ||//bsSm3
					( bsLTrg3==1 && pair[i]->pT()>2.8)//bsLg3
				){ ptThdCut = 1;}
				...
				//}//added by Xiaoxuan, require pt1>threshold for each trigger, ends

				//cout<< "before loop 2nd pair"<<endl;
				//loop over 2nd pair
				for(unsigned int j=i+1; j<pair.size(); j++){
					...
					//cout<<" ptbin1<0 continue "<<endl;
					int id2=pair[j]->point(0)->id();
					int id3=pair[j]->point(1)->id();
					if(id0==id2 || id0==id3 || id1==id2 || id1==id3) continue; //using same photon
					//cout<<" using same photon continue "<<endl;
					float dphi = wrapAround(pair[i]->phi() - pair[j]->phi());
					if(pair[i]->eta()<2.6||pair[i]->eta()>4.0||pair[j]->eta()<2.6||pair[j]->eta()>4.0)continue;

					double x1 = (pair[i]->pT()*pow(TMath::E(),pair[i]->eta())+pair[j]->pT()*pow(TMath::E(),pair[j]->eta()))/200.;
                                        double x2 = (pair[i]->pT()*pow(TMath::E(),-1*pair[i]->eta())+pair[j]->pT()*pow(TMath::E(),-1*pair[j]->eta()))/200.;
					...
					//if(ag<=3 && bsTrg==1     && exc1) cut2[17]=1;//Xiaoxuan, include all ag
					...
					//Xiaoxuan, trigger depedence
					if(ag==0 && bsSTrg1==1    && exclusive1) cut2[20]=1;
                                        if(ag==0 && bsSTrg2==1    && exclusive1) cut2[21]=1;
                                        if(ag==0 && bsSTrg3==1    && exclusive1) cut2[22]=1;
                                        if(ag==0 && bsLTrg1==1    && exclusive1) cut2[23]=1;
                                        if(ag==0 && bsLTrg2==1    && exclusive1) cut2[24]=1;
                                        if(ag==0 && bsLTrg3==1    && exclusive1) cut2[25]=1;
					//Xiaoxuan
					...
					//cout<<" before fill hist"<<endl;
					//added bu Xiaoxuan, require pt1>threshold for each trigger
					//if( ptThdCut == 1){
						//filling histos
						for(unsigned int k=0; k<kNCut; k++){
							if(cut2[k]==1){
								...
								mPhi2[ptbin0][ptbin1][k]->Fill(p1);
							        cout<<" before fill dphi "<<endl;
								mDphi[ptbin0][ptbin1][k]->Fill(dphi);
							//	mX1[ptbin0][ptbin1][k]->Fill(x1);
                                        		//	mX2[ptbin0][ptbin1][k]->Fill(x2);
								cout<<" dphi "<<dphi<<endl;
								mBbce[ptbin0][ptbin1][k]->Fill(bbcEsum);
								mPhi1Dphi[ptbin0][ptbin1][k]->Fill(p0,dphi);
								//mBXingDphi[ptbin0][ptbin1][k]->Fill(bunch,dphi);
							}
						} 	    
				    	//}//added by Xiaoxuan, require pt1>threshold for each trigger,ends
		...
		if(n>1){
			for(int i=0; i<n; i++){
				...
				for(int j=i+1; j<n; j++){
					...
					//if(fmssim->pi0(i)->pseudoRapidity()>2.6&&fmssim->pi0(i)->pseudoRapidity()<4.0&&fmssim->pi0(j)->pseudoRapidity()>2.6&&fmssim->pi0(j)->pseudoRapidity()<4.0){
					double x1 = (fmssim->pi0(i)->perp()*pow(TMath::E(),fmssim->pi0(i)->pseudoRapidity())+fmssim->pi0(j)->perp()*pow(TMath::E(),fmssim->pi0(j)->pseudoRapidity()))/200.;
					double x2 = (fmssim->pi0(i)->perp()*pow(TMath::E(),-1*fmssim->pi0(i)->pseudoRapidity())+fmssim->pi0(j)->perp()*pow(TMath::E(),-1*fmssim->pi0(j)->pseudoRapidity()))/200.;
					mX1->Fill(x1);
					mX2->Fill(x2);
//}
					...
					//mX1[ptbin0][ptbin1][kNCut]->Fill(x1);
                                        //mX2[ptbin0][ptbin1][kNCut]->Fill(x2);
